...
let rule_getInputOfConclusion (Rule(_,Formula(predname,ts))) = 
	if ts = [] then raise(Failure("Found a predicate name without any arguments: " ^ predname)) else 
	if predname = "typeOf" then List.nth (List.rev ts) 1 else List.hd ts   (* old for typeOf was List.nth ts 1, now second last. *)
...
let language_getValueArgs lan cname : term list = 
	let allValues = language_grammarLookupByCategory lan "Value" in 
	let itemsOfCNAMEinValue : term list = List.filter (fun t -> compareConstructors t (Constr(cname,[]))) allValues in (* no reason there should be more than one, but we retrieve all and then the first one. *)
	if itemsOfCNAMEinValue = [] then raise(Failure("language_getValueArgs: found no grammar item for that value constructor in the Value grammar")) else 
	let itemOfCNAME = List.hd itemsOfCNAMEinValue (* just take the first *) in 
...
let language_getTypeByOp lan cname : term =   
	...
	let rulesOfCNAME = List.filter (rule_isPredname "typeOf") allRulesOfCNAME in 
	if rulesOfCNAME = [] then raise(Failure("language_getTypeByOp: found no rules for that constructor")) else 
	let ruleOfCNAME = List.hd rulesOfCNAME (* just take the first *) in 
...
let lang_retrieveTHERule_by_predname_opname lan (predname: string) (opname: string) : rule =
    let rules = lang_retrieveRules_by_predname_opname lan predname opname in
    if rules = [] then raise (Failure("lang_retrieveRule_by_predname_opname: did not find " ^ predname ^ " and " ^ opname)) else List.hd rules

	(* list_to_numbered_list transforms [a0,a1,a2] in [(0,a0), (1,a1), (2,a2)] *)
let list_to_numbered_list l = mapi (fun i a -> (i,a)) l		

let rec list_take n l = if n = 0 || l = [] then [] else (List.hd l) :: (list_take (n-1) (List.tl l))

	(* filter_maintain_position_in_list transforms [a0,a1,a2,e3] in [None, Some a1, None, Some e3] if a1 and a3 satisfies the given property. *)
...
let lang_getIdxOfOnlyPremisesOfPredname (premises : formula list) (predname : string) : int list = 
	let list_with_Some_and_None = filter_maintain_position_in_list premises (fun f -> formula_getPredname f = predname) in 
	let that_list_with_numbers = list_to_numbered_list list_with_Some_and_None in 
	List.map fst (List.filter (fun (i,o) -> is_some o) that_list_with_numbers)

let language_get_strong_maps lan : term list = 
	let findStrongMaps grammarLine = if is_none (grammarLine_getItemsOption grammarLine) then false else List.exists (fun t -> term_isConstr t && term_getCNAME t = lang_map_constructor_strong) (get (grammarLine_getItemsOption grammarLine)) in 
	let grammarLines = List.filter findStrongMaps (language_getGrammar lan) in 
	if grammarLines = [] then [] else 
		let result = List.map (fun gl -> LangVar (get (grammarLine_getMetavarOption gl))) grammarLines in 
		(* let _ = print_string ("language_get_strong_maps: " ^ dump result) in *)
		result
