...
typeOf Gamma EnvH EnvR (jmp E) (unitT) :- 
	typeOf Gamma EnvH EnvR E (refH (codeT T1 T2)), 
	retrieveEnvR EnvR (r1) T1 (r2) T2. 

typeOf Gamma EnvH EnvR (bnz Zs E1 E2) (unitT) :- 
	typeOneZ Gamma EnvR Zs (int), 
	typeOf Gamma EnvH EnvR E1 (refH (codeT T1 T2)), 
	typeOf Gamma EnvH EnvR E2 (unitT), 
	retrieveEnvR EnvR (r1) T1 (r2) T2. 

typeOf Gamma EnvH EnvR (code T1 T2 E) (codeT T1 T2) :- 
	typeOf Gamma EnvH EnvR' E (unitT), 
	setEnvR (r1) T1 (r2) T2 EnvR'. 
...
step (jmp (labelH L)) H R E H R :- 
	lookupMapH H L (code T1 T2 E).

step (bnz Zs (labelH L) E2) H R E1 H R :- 
	lookupMapR R Zs (zero), 
	lookupMapH H L (code T1 T2 E1).
