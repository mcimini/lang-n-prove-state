...
let print_equalities (equalitiesOpt : ((var * evaluatedExpression) list) option) = if is_none equalitiesOpt then "" else let equalities = get equalitiesOpt in 
	" with " ^ String.concat ", " (List.map (fun (var,t) -> var ^ "=" ^ print_evalExp t) equalities)
...
let rec print_proof proof = match proof with 
	...
	| Apply(lnp_name1, lnp_name2, lnp_names, equalities) -> print_lnp_name lnp_name1 ^ ": " ^ "apply " ^ print_lnp_name lnp_name2 ^ " to " ^ (String.concat " " (List.map print_lnp_name lnp_names)) ^ print_equalities equalities
